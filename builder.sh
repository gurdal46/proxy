...
function usage() { echo "" 1>&2; exit 1; }
...
function open_ufw_backconnect_ports(){
  ...
  if ! is_package_installed "ufw"; then echo ""; return; fi;
...
function run_proxy_server(){
  if [ ! -f $startup_script_path ]; then log_err_and_exit ""; fi;
...
if [ $uninstall = true ]; then
  ...
  rm -rf $proxy_dir;
  delete_file_if_exists $backconnect_proxies_file;
  echo -e "proxy server successfully uninstalled.";
  exit 0;
fi;
...
if is_proxyserver_installed; then
  echo -e "Proxy server already installed";
else
  configure_ipv6;
